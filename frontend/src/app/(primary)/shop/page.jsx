...
import BoxProduct from '../../../components/BoxProduct'; // Assuming this component is correctly defined
...
const Shop = ({ searchParams }) => {
  ...
  const handleSortChange = (event) => {
    ...
    let sortField = 'name_pr'; // Default sorting by name
    let sortOrder = '';
        ...
        <div className="mb-4">
          <ol className="flex items-center whitespace-nowrap">
            <li className="inline-flex items-center">
              <a className="flex items-center text-sm text-gray-500 hover:text-orange-200 focus:outline-none focus:text-blue-600 dark:text-neutral-500 dark:hover:text-blue-500 dark:focus:text-blue-500" href="/">
                Home
              </a>
        ...
        <div className='flex lg:flex-row flex-col-reverse gap-10 mb-20'>
              ...
              {loading ? (
                ...
                products.length === 0 ? (
                  ...
                  products.map((product) => (
                    <BoxProduct key={product._id} product={product} color="text-black" />
                  ))
                )
              )}
            </div>
          ...
          <div className='lg:w-1/4 lg:block hidden px-4 text-black'>
            <div className='mb-4'>
              ...
              <div className="form-control">
                {categories.map((category, i) => (
                  <a href={`/shop?tagname=${category.tag_ct}`} className="cursor-pointer label mb-2 flex justify-between" key={category.id}>
                    <span className="label-text text-text_1 hover:text-primary">{category.name_ct}</span>
                    <p>({category.count})</p>
                  </a>
                ))}
              </div>
            ...
            <div className='mb-4'>
              <img src={`http://localhost:4000/images/shop_2.png`} alt="" className='w-full object-cover h-[300px]' />
            </div>
            <div className='mb-4'>
              <div className='font-bold text-xl mb-2'>Filter By Price</div>
              <Shop_Filter_Price onApplyFilter={handleApplyFilter} />
